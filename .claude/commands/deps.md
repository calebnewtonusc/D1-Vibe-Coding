...
Check dependency health for `$ARGUMENTS` (or current directory).
...
- **Unused**: packages safe to remove (with `npm uninstall` command ready)
- **Outdated**: version delta, major/minor/patch classification

Suggest specific fix commands for the most critical issues.
